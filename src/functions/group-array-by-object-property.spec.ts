...
describe('groupArrayByObjectProperty', () => {
    ...
    it('on A', () => {
        expect.assertions(1);
        const result = groupArrayByObjectProperty(
            array,
            (element) => element.a,
        );
    ...
    it('on C', () => {
        ...
        expect(result).toStrictEqual({
            c: [{ a: 1, b: '0', c: 'c', d: new Date(6000) }],
            e: [
                { a: 1, b: '2', c: 'e', d: new Date(5000) },
                { a: 1, b: '2', c: 'e', d: new Date(8000) },
            ],
            p: [{ a: 2, b: '2', c: 'p', d: new Date(7000) }],
            t: [{ a: 3, b: '0', c: 't', d: new Date(8000) }],
            x: [{ a: 2, b: '0', c: 'x', d: new Date(6000) }],
        });
    });

    it('on D as number', () => {
        expect.assertions(1);
        const result = groupArrayByObjectProperty(array, (element) =>
            element.d.getTime(),
        );

        expect(result).toStrictEqual({
            5000: [{ a: 1, b: '2', c: 'e', d: new Date(5000) }],
            6000: [
                { a: 2, b: '0', c: 'x', d: new Date(6000) },
                { a: 1, b: '0', c: 'c', d: new Date(6000) },
            ],
            7000: [{ a: 2, b: '2', c: 'p', d: new Date(7000) }],
            8000: [
                { a: 1, b: '2', c: 'e', d: new Date(8000) },
                { a: 3, b: '0', c: 't', d: new Date(8000) },
            ],
